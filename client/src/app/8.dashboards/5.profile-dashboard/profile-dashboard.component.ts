...
import { RouterModule, ActivatedRoute, Router } from '@angular/router'; // Add RouterModule and ActivatedRoute
...
export class ProfileDashboardComponent implements OnInit {
  userData!: ILoginDetail;
  editUserData!: ILoginDetail;
  ...
  getUserData(): void {
    ...
    if (storedUser) {
      ...
      this.logindetailService.getLoggedInUser().subscribe(
        (data) => {
  ...
  saveProfile(): void {
    console.log('Updated User Data:', this.editUserData);
    this.logindetailService.edit(this.editUserData).subscribe(
      (updatedData) => {
  ...
  // ✅ FIX: Add this function to handle file selection
  onFileSelected(event: any): void {
    const file: File = event.target.files[0]; // Get the selected file
    if (file) {
      console.log('Selected file:', file);
      // You can store the file in a variable if needed for further processing
    }
  }
}
