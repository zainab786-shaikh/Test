...
export class ControllerLesson extends BaseController {
  ...
  async getExplanation(@request() req: Request, @response() res: Response) {
    try {
      ...
      const lesson = await this.serviceLesson.get(lessonId);
      this.logger.info("Retrieved Explanation:" + lesson?.Explanation);
      ...
      res.status(HttpStatusCode.OK).json(lesson?.Explanation);
    } catch (error: any) {
  ...
  async getQuiz(@request() req: Request, @response() res: Response) {
    try {
      const lessonId = +req.params.Id;
      const lesson = await this.serviceLesson.get(lessonId);
      this.logger.info("Retrieved Explanation:" + lesson?.Quiz);
      ...
      res.status(HttpStatusCode.OK).json(lesson?.Quiz);
    } catch (error: any) {
      this.logger.error(error);
      return this.handleError(error, res);
    }
  ...
  async getFillBlank(@request() req: Request, @response() res: Response) {
    try {
      const lessonId = +req.params.Id;
      const lesson = await this.serviceLesson.get(lessonId);
      this.logger.info("Retrieved Explanation:" + lesson?.FillBlanks);
      ...
      res.status(HttpStatusCode.OK).json(lesson?.FillBlanks);
    } catch (error: any) {
      this.logger.error(error);
      return this.handleError(error, res);
    }
  ...
  async getTrueFalse(@request() req: Request, @response() res: Response) {
    try {
      const lessonId = +req.params.Id;
      const lesson = await this.serviceLesson.get(lessonId);
      this.logger.info("Retrieved Explanation:" + lesson?.TrueFalse);
      ...
      res.status(HttpStatusCode.OK).json(lesson?.TrueFalse);
    } catch (error: any) {
      this.logger.error(error);
      return this.handleError(error, res);
    }
